...
import { quzzySearch, QuzzySearchOptions } from "./quzzySearch.js";
...
export class BibleLibrary {
  ...
  private _selectedEditions: string[] = [];
  ...
  searchBookName(query: string, options?: QuzzySearchOptions): BookSearchResult[] {
    ...
    return quzzySearch<BookSearchResult>(query, ["name"], selectedBooks, options)
  }
  ...
  getBook(query: string | number, edition?: BollsBible.Translation['short_name'], options?: QuzzySearchOptions): BollsBible.Book {
    if (typeof query == 'number') {
      if (!edition) throw new Error('You must provide translation short name to retrieve a book by id.')
      else {
      ...
      if (bookNameSearchResult.length) {
        if (edition) {
          let result: BollsBible.Book[] = bookNameSearchResult.filter(book => book.translation == edition);
        ...
        } else return bookNameSearchResult[0]
      }
    }
